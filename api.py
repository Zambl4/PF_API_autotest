#Класс с методами АПИ + 2 новых метода описаны самостоятельно
...
class PetFriends:
    ...
    def create_pet_simple (self, auth_key: json, name: str, animal_type: str, age: int) -> json:
        """Мой новый метод - отправляет на сервер запрос с данными питомца без фото
    ...
    def add_photo_of_pet(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Мой новый метод - отправляет на сервер фото питомца по id и возвращает статус
        запроса на сервер и результат в формате JSON с данными добавленного питомца"""

        #возможно данные нужно передать так
        data = MultipartEncoder(
            fields={
                'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'image/jpeg')
            })
